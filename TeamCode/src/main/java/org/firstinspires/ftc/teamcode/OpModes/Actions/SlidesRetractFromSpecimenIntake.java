...
public class SlidesRetractFromSpecimenIntake implements Action {
    private IntakeSlide intakeSlide;
    private boolean initialized = false;
    private Telemetry telemetry;


    public SlidesRetractFromSpecimenIntake(IntakeSlide intakeSlide, Telemetry telemetry){
    ...
    public boolean run(@NonNull TelemetryPacket telemetryPacket) {
        ...
        if (!initialized) {
            intakeSlide.retractSlideFromSpecIntake();
            initialized = true;
        }
        ...
        if (pos > intakeSlide.SLIDE_SPEC_INTAKE_DELTA + 50) {
            ...
            telemetry.addLine("Slides reached specimen");
            telemetry.update();
